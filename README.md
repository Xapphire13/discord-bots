...
- Creates the `/opt/<bot-name>/` directory on the remote host
...
- A systemd service configured on the target host for each bot
- The target directory `/opt/<bot-name>/` will be created automatically
...
4. Installs files to `/opt/<bot-name>/`
